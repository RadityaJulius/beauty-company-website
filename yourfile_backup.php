...
<script>
    ...
    .then(data => {
        ...
        let costumers = data1.map(costumer => ({
            ...
            nama_treatment: costumer.nama_treatment
        }));
        ...
        costumers.forEach((costumer, index) => {
            ...
            card.innerHTML = `
                    <div class="bg-white rounded-lg shadow-md p-6 max-w-md mx-auto border">
                        <h3 class="text-lg font-semibold text-gray-800 mb-4">${costumer.nama_treatment}</h3>
                        <p class="text-gray-600" id="namaCust" value="${costumer.id}"><strong>Name: </strong><span class="userName">${costumer.nama}</span></p>
                        ...
                        <p class="text-gray-600"><strong>Service: </strong><span class="userPhone">${costumer.nama_treatment}</span></p>
                        <p class="text-gray-600"><strong>Price: </strong>Rp. ${parseInt(costumer.harga).toLocaleString('id-ID')}</p>
                        <p class="text-gray-600"><strong>Tanggal Treatment: </strong>${costumer.tanggal_treatment}</p>
                        <div class="flex gap-2 mt-4">
                            <button class="delete-card bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-yellow-500">
                                Delete
                            </button>
            ...
            card.querySelector(".delete-card").addEventListener("click", () => {
                const custName = card.querySelector(".userName").textContent;
                console.log(custName)

                const form = document.createElement('form')
                form.setAttribute('method', 'post')
                form.setAttribute('action', 'delete_card.php')
                form.style.display = 'hidden'

                input = document.createElement('input')
                input.setAttribute('type', 'hidden')
                input.setAttribute('name', 'custName')
                input.setAttribute('value', custName)
        ...
        function markAsPaid(card) {
            card.querySelector(".delete-card").remove();
